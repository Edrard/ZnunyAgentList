package Kernel::GenericInterface::Operation::Ticket::Close;
...
sub Run {
    ...
    if (!$StateData) {
        push @Errors, 'Target close state not found.';
    }
    elsif ( !Kernel::GenericInterface::Operation::ZnunyAgentList::Common->IsClosedStateType( $StateData->{StateType} ) ) {
        push @Errors, 'Target close state must have a closed state type.';
    }
    ...
    if (!$ArticleID) {
        return {
            ...
            Data    => {
                Ticket   => $Ticket,
                Errors   => ['Close article could not be created.'],
                Warnings => \@Warnings,
            },
        };
    }
